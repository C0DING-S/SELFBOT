...
system("title " + "SCREAMZ SELFBOT [github.com/C0DING-S]")
banner1 = (Fore.BLUE + """
C0DING-S""")
...
RPC.update(
    ...
    start=cur_time,
    buttons=[{"label": "GET IT YOURSELF", "url": "https://github.com/C0DING-S/SCREAMZ-SELFBOT"}])
...
async def on_ready():
    ...
    print(Fore.CYAN + "\t\t\t\t\t\tgithub.com/C0DING-S")
    print(Fore.GREEN + f"\t\t\t\t\t\t Start with {prefix}help")


@bot.command()
async def help(ctx):
    embed = discord.Embed(title="C0DING-S",
                          description="https://github.com/C0DING-S",
                          color=color,
...
async def give_role(ctx, user: discord.Member = "", role=""):
    if user == "" or role == "":
        embed = discord.Embed(title=f"Use command like this: {prefix}give_role C0DING-Sadmin", colour=color)
        await ctx.send(embed=embed)
        return
...
async def nick(ctx, user: discord.Member = None, *, name=""):
    if name == "" or user is None:
        embed = discord.Embed(title=f"Use command like this: {prefix}nick C0DING-S king", colour=color)
        await ctx.send(embed=embed)
        return
    try:
...
async def info(ctx):
    ...
    embed.add_field(name="Author", value="https://github.com/C0DING-S")
...
async def nuke(ctx):
    ...
    if nuker:
        ...
        while True and stop:
            await ctx.message.guild.create_text_channel("nuked by github.com/C0DING-S")
...
async def role_spam(ctx):
    if nuker:
        ...
        while True and stop:
            await ctx.message.guild.create_role(name="C0DING-S", colour=discord.Colour.blue())
        else:
            stop = True
